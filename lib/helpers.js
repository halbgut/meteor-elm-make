...
helpers = (Plugin, Meteor) => {
  ...
  const getNativeModulePath = (filePath, module, index) => {
    if(index) {
      return path.join(module, filePath.substr(0, filePath.length - 7) + '.js')
    } else {
  ...
  const isIndexModule = (packageName, filename) => makeModuleName(packageName) === filename.substring(0, filename.length - 4)

  const isNativeIndex = (packageName, filename) => makeModuleName(packageName) === filename.substring(0, filename.length - 7)
